...
func TestTryOIDCEmailFastPathCreatesUserAndIdentity(t *testing.T) {
	handler, client := newOAuthPendingFlowTestHandler(t, false)
	t.Cleanup(func() { _ = client.Close() })
	...
	completed := handler.tryOIDCEmailFastPath(
		c,
		"/auth/oidc/callback",
		"/dashboard",
		identity,
...
func TestOIDCOAuthCallbackEmailFastPathIssuesTokenWithoutPendingSession(t *testing.T) {
	cfg, cleanup := newOIDCTestProvider(t, oidcProviderFixture{
...
func TestOIDCOAuthCallbackEmailFastPathBackendModeBlocksBeforeUserCreation(t *testing.T) {
	cfg, cleanup := newOIDCTestProvider(t, oidcProviderFixture{
...
func TestTryOIDCEmailFastPathBypassesInvitationCodeRequirement(t *testing.T) {
...
func TestTryOIDCEmailFastPathBypassesForceEmailSignup(t *testing.T) {
...
func TestTryOIDCEmailFastPathBypassesRegistrationEmailSuffixWhitelist(t *testing.T) {
	handler, client := newOAuthPendingFlowTestHandlerWithDependencies(t, oauthPendingFlowTestHandlerOptions{
		settingValues: map[string]string{
			service.SettingKeyRegistrationEmailSuffixWhitelist: `["@allowed.example.com"]`,
		},
	})
	t.Cleanup(func() { _ = client.Close() })
	...
	identity := service.PendingAuthIdentityKey{
		ProviderType:    "oidc",
		ProviderKey:     "https://issuer.example.com",
		ProviderSubject: "fast-path-bypasses-email-policy",
	}
	completed := handler.tryOIDCEmailFastPath(
		c,
		"/auth/oidc/callback",
	...
	userCount, err := client.User.Query().Where(dbuser.EmailEQ("external@example.org")).Count(context.Background())
	require.NoError(t, err)
	require.Equal(t, 1, userCount)
}
